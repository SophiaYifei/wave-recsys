...
| `hyperparam_sweep.json` | `scripts/experiment.py --type=hyperparam_sweep` | Test metrics for six configurations: `embed_dim ∈ {64, 128, 256, 512}` at `max_epochs=40`, plus `embed_dim=128` at `max_epochs ∈ {20, 60}`. Best config (64/40) is promoted as the canonical `model.pt`. |
